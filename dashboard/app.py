"""
NetGuard IoT — Streamlit SOC Dashboard
====================================
"""
...
FALLBACK_THREAT_EXPLANATION = (
    "ALERT: Device 172.20.0.99 exhibits behavior consistent with network reconnaissance. "
)

DEFAULT_DEVICES = {
    "172.20.0.10": {"name": "IP Camera", "ip": "172.20.0.10", "pps": 0.5, "avg_pkt": 120.0, "unique_ports": 1, "confidence": 95.2, "protocol_diversity": 1.2, "burst_score": 0.3, "status": "NORMAL", "mac": "0e:75:0e:1a:02:9f", "icon": "📷", "port": "GigabitEthernet0/10"},
    "172.20.0.11": {"name": "Thermostat", "ip": "172.20.0.11", "pps": 0.03, "avg_pkt": 35.0, "unique_ports": 1, "confidence": 98.1, "protocol_diversity": 1.0, "burst_score": 0.1, "status": "NORMAL", "mac": "76:2d:b4:87:f4:28", "icon": "🌡️", "port": "GigabitEthernet0/11"},
    "172.20.0.12": {"name": "Smart Bulb", "ip": "172.20.0.12", "pps": 0.02, "avg_pkt": 18.0, "unique_ports": 1, "confidence": 97.5, "protocol_diversity": 1.0, "burst_score": 0.05, "status": "NORMAL", "mac": "92:4d:cd:1f:66:02", "icon": "💡", "port": "GigabitEthernet0/12"},
    "172.20.0.99": {"name": "UNKNOWN", "ip": "172.20.0.99", "pps": 150.0, "avg_pkt": 42.0, "unique_ports": 847, "confidence": 2.3, "protocol_diversity": 5.1, "burst_score": 15.7, "status": "ROGUE", "mac": "6a:62:94:dd:96:f4", "icon": "❓", "port": "GigabitEthernet0/24"}
}
...
def _check_ollama():
    ...
    try: return requests.get(OLLAMA_URL, timeout=OLLAMA_TIMEOUT_SECONDS).status_code == 200
    except: return False
...
def _get_simulated_devices() -> List[Dict[str, Any]]:
    ...
    df = None
    if not csv_empty:
        try: df = pd.read_csv(csv_path).tail(1000)
        except: pass
        ...
        if not df_filtered.empty:
            ...
            try:
                for src_ip, group in df_filtered.groupby('src_ip'):
                    ...
                    if _predict_available: predict.reset_stats(src_ip)
                    ...
                    def mock_time():
                        nonlocal packet_idx
                        try: caller = sys._getframe(1).f_code.co_name
                        except: caller = ""
                    ...
                    if _predict_available: predict.time.time = mock_time
                    ...
                    for idx, (_, row) in enumerate(group_sorted.iterrows()):
                        if _predict_available:
                            ...
                            last_prediction = predict.predict_device(
                                src_ip,
                                int(row['packet_size']),
                                int(row['dst_port']),
                                str(row['protocol']),
                    ...
                    if _predict_available: predict.time.time = orig_time_func
                    ...
                    if uniq_ports > 50 or pps > 20: status = "ROGUE"
                    
                    d_obj = DEFAULT_DEVICES.get(src_ip, {})
                    devices_dict[src_ip] = {
                        "name": d_obj.get("name", "UNKNOWN"),
                        "ip": src_ip,
                        "pps": round(pps, 2),
                        "avg_pkt": float(group_sorted['packet_size'].mean()),
                        "unique_ports": uniq_ports,
                        "confidence": last_prediction.get("confidence", 0.0),
                        "protocol_diversity": float(group_sorted['protocol'].nunique()),
                        "burst_score": 0.0,
                        "status": status,
                        "mac": d_obj.get("mac", "00:15:5d:00:00:00"),
                        "icon": d_obj.get("icon", "❓"),
                        "port": d_obj.get("port", "GigabitEthernet0/48")
                    }
            except Exception as e:
                logger.error(f"Live stats error: {e}")
                if _predict_available: predict.time.time = orig_time_func
        else: st.session_state["data_source_live"] = False
    else: st.session_state["data_source_live"] = False
...
def generate_incident_report(device):
    ...
    try:
        if requests is not None:
            ...
            if res.status_code == 200: report_text = res.json().get("response", "")
    except Exception:
        pass
...
def async_quarantine(ip: str):
    if st.session_state.get("quarantine_status", {}).get(ip) in ["in_progress", "completed"]: return
...
def _init_session_state():
    defaults = {
        "start_time": datetime.now(), "threats_detected": 0, "devices_quarantined": 0,
        "quarantine_actions": [], "history": {}, "audit_logs": [], "data_source_live": False,
        "quarantine_status": {}, "auto_resolved_ips": set(), "generated_reports": {}
    }
...
def render_live_dashboard(auto_quarantine):
    ...
    # Update history for Moving graph
    for d in devices:
        ip_addr = d["ip"]
    ...
    for d in devices:
        ...
        if q_stat == "in_progress":
            ...
            if d["status"] == "ROGUE": action_html = '<span class="badge badge-red">Needs Action</span>'
        ...
        # Animated PPS text (CSS handles simple fade or just live change)
    ...
    with c1:
        ...
        if rogues:
            ...
            if st.button("📧 Send Email Alert"):
                try:
                    msg = EmailMessage(); msg.set_content(rep['text']); msg["Subject"]=f"Alert: {rep['id']}"; msg["From"]="soc@netguard.local"; msg["To"]=ADMIN_EMAIL
                    s = smtplib.SMTP("localhost", 2525); s.send_message(msg); s.quit()
                    st.success(f"Email sent successfully to {ADMIN_EMAIL}!")
                except: st.info(f"Report emailed to {ADMIN_EMAIL} (simulated; no smtp server)")
        else:
    ...
    try:
        ...
        if os.path.exists(csv_path) and os.path.getsize(csv_path) > 0:
            ...
            for _, r in df_live.iterrows():
                ...
                src_ip = str(r['src_ip'])
                dst_ip = str(r['dst_ip'])
                if "172.20.0.99" in [src_ip, dst_ip]: color = "#ff4444" # Rogue red
                elif "172.20.0" in src_ip and DEFAULT_DEVICES.get(src_ip, {}).get("status") == "SUSPICIOUS": color = "#ffcc00"
