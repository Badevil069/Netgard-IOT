...
DEVICE_CATALOG: dict[str, dict[str, str]] = {'172.20.0.10': {'name': 'IP Camera', 'mac': '0e:75:0e:1a:02:9f', 'icon': 'camera'}, '172.20.0.11': {'name': 'Thermostat', 'mac': '76:2d:b4:87:f4:28', 'icon': 'thermometer'}, '172.20.0.12': {'name': 'Smart Bulb', 'mac': '92:4d:cd:1f:66:02', 'icon': 'lightbulb'}, '172.20.0.99': {'name': 'UNKNOWN', 'mac': '6a:62:94:dd:96:f4', 'icon': 'shield-alert'}}
...
def _run_prediction_for_group(group: pd.DataFrame) -> dict[str, Any]:
    ...
    if packet_count > 0:
        ...
        with contextlib.redirect_stdout(io.StringIO()):
            ...
            for idx, (_, row) in enumerate(rows.iterrows()):
                ...
                last_result = predict.predict_device(ip=ip, packet_size=int(row['packet_size']), dst_port=int(row['dst_port']), protocol=str(row['protocol']), timestamp=ts_float, run_prediction=is_last)
    ...
    return {'name': catalog['name'], 'ip': ip, 'mac': catalog['mac'], 'status': 'QUARANTINED' if quarantined else status, 'pps': round(observed_pps, 2), 'unique_ports': unique_ports, 'avg_packet_size': avg_packet_size, 'confidence': confidence, 'quarantined': quarantined, 'last_seen': _format_timestamp(latest_seen), 'detection_reasons': _detection_reasons(observed_pps, unique_ports, score, catalog['name'], quarantined), 'raw_score': round(score, 4)}
...
def _packets_to_json(frame: pd.DataFrame, limit: int=50) -> list[dict[str, Any]]:
    ...
    for row in tail.to_dict(orient='records'):
        ...
        if pd.notna(timestamp):
            ...
            timestamp_value = ''
        packets.append({'timestamp': timestamp_value, 'src_ip': str(row.get('src_ip', '')), 'dst_ip': str(row.get('dst_ip', '')), 'src_port': int(row.get('src_port', 0) or 0), 'dst_port': int(row.get('dst_port', 0) or 0), 'protocol': str(row.get('protocol', '')), 'packet_size': int(row.get('packet_size', 0) or 0)})
    return packets
...
def _read_incident_reports() -> list[dict[str, Any]]:
    ...
    for path in sorted(glob.glob(str(INCIDENT_DIR / '*.txt')), reverse=True):
        ...
        reports.append({'id': parsed.get('id', Path(path).stem), 'device': parsed.get('device', 'unknown'), 'threat': parsed.get('threat', 'unknown'), 'type': parsed.get('type', 'unknown'), 'affected': parsed.get('affected', 'unknown'), 'action': parsed.get('action', 'unknown'), 'timestamp': parsed.get('timestamp', 'unknown'), 'raw': content, 'path': Path(path).name})
    return reports
...
def _summary_stats(devices: list[dict[str, Any]], packets: pd.DataFrame, incidents: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    if rogue_devices > 0:
        ...
        threat_level = 'GREEN'
    return {'total_devices': total_devices, 'normal_devices': normal_devices, 'rogue_devices': rogue_devices, 'quarantined_devices': quarantined_devices, 'total_packets': total_packets, 'incident_count': len(incidents), 'threat_level': threat_level, 'system_status': 'ONLINE', 'timestamp': dt.datetime.now(dt.timezone.utc).isoformat()}
...
def _device_by_ip(ip: str) -> dict[str, Any]:
    ...
    for device in devices:
        if device['ip'] == ip:
            return device
    catalog = _device_catalog_entry(ip)
    return {'name': catalog['name'], 'ip': ip, 'mac': catalog['mac'], 'status': 'NORMAL', 'pps': 0.0, 'unique_ports': 0, 'avg_packet_size': 0.0, 'confidence': 0.0, 'quarantined': any((entry.get('ip') == ip for entry in get_quarantine_log())), 'last_seen': dt.datetime.now(dt.timezone.utc).isoformat(), 'detection_reasons': [], 'raw_score': 0.0}
...
def get_packets() -> dict[str, Any]:
    frame = _load_csv()
    total_packets = _count_lines(CSV_PATH)
    return {'packets': _packets_to_json(frame, limit=50), 'total_packets': total_packets, 'timestamp': dt.datetime.now(dt.timezone.utc).isoformat()}
